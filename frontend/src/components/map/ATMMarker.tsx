...
export function ATMMarker({ id, position, name, address, risk_score, incident_count }: ATMMarkerProps) {
  const icon = useMemo(() => {
    let colorClass = "text-[#22c55e] bg-[#141f2a] border-[#22c55e]";
    if (risk_score > 0.7) {
      colorClass = "text-[#ef4444] bg-[#141f2a] border-[#ef4444]";
    } else if (risk_score >= 0.3) {
      colorClass = "text-[#f59e0b] bg-[#141f2a] border-[#f59e0b]";
    }
  ...
  const riskColor = risk_score > 0.7 ? "text-[#ef4444]" : risk_score >= 0.3 ? "text-[#f59e0b]" : "text-[#22c55e]";
    ...
    <Marker position={position} icon={icon}>
      <Popup>
        <div className="min-w-[200px]">
          ...
          <p className="mt-0.5 text-xs text-slate-400">{address}</p>
          ...
          <dl className="grid grid-cols-2 gap-2 text-xs">
            ...
            <div className="flex flex-col gap-0.5">
              ...
              <dd className="font-mono font-medium text-slate-100">{incident_count}</dd>
            </div>
          </dl>
        </div>
      </Popup>
