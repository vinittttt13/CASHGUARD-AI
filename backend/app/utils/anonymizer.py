...
def mask_text(text: str) -> str:
    ...
    text = re.sub(r'(?i)\b(?:a/c|acct|account|acc)[\s#.:no]*([0-9]{9,18})\b', _mask_acct_match, text)
...
def mask_complaint_data(complaint_dict: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "complaint_text" in masked_data and isinstance(masked_data["complaint_text"], str):
        masked_data["complaint_text_masked"] = mask_text(masked_data.pop("complaint_text"))
