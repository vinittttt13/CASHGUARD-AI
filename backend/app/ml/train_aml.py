...
PROJECT_ROOT = Path(__file__).resolve().parents[2]  # backend root, or parents[3] workspace
...
def load_aml_dataset(csv_path: Path, max_rows: int = 250_000, include_patterns: bool = True) -> pd.DataFrame:
    """
    ...
    target_col = 'Is Laundering' if 'Is Laundering' in df_trans.columns else 'Is_Laundering'
    df_trans[target_col] = df_trans[target_col].astype(int)
    laundering_count = int(df_trans[target_col].sum())
    logger.info("Sampled %d transactions uniformly across the full dataset (%d laundering rows = %.2f%%)",
                len(df_trans), laundering_count, (laundering_count / len(df_trans)) * 100)
    ...
    if include_patterns and PATTERNS_FILE.exists():
        logger.info("Enriching with ground truth laundering patterns from %s", PATTERNS_FILE)
        df_patterns = parse_patterns_file(PATTERNS_FILE)
        if not df_patterns.empty:
            logger.info("Extracted %d ground-truth laundering transactions from patterns", len(df_patterns))
            df_combined = pd.concat([df_trans, df_patterns], ignore_index=True)
            df_combined = df_combined.drop_duplicates(
                subset=['Timestamp', 'From Bank', 'Account', 'To Bank', 'Account.1', 'Amount Paid']
            )
            df_trans = df_combined
            new_laundering = int(df_trans[target_col].sum())
            logger.info("Total after pattern enrichment: %d transactions (%d laundering, %0.2f%%)",
                        len(df_trans), new_laundering, (new_laundering / len(df_trans)) * 100)
...
def train_aml_model(
    ...
    logger.info("Train Laundering: %d / %d (%.2f%%)", y_train.sum(), len(y_train), (y_train.sum()/len(y_train))*100)
    logger.info("Val Laundering:   %d / %d (%.2f%%)", y_val.sum(), len(y_val), (y_val.sum()/len(y_val))*100)
    logger.info("Test Laundering:  %d / %d (%.2f%%)", y_test.sum(), len(y_test), (y_test.sum()/len(y_test))*100)
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--csv", type=str, default=str(DEFAULT_DATASET), help="Path to AML transactions CSV")
    parser.add_argument("--max-rows", type=int, default=150_000, help="Max rows to train on (default: 150,000)")
    parser.add_argument("--model-version", type=str, default="v1.0", help="Model version tag")
    args = parser.parse_args(argv)

    csv_path = Path(args.csv)
    if not csv_path.exists():
        logger.error("Dataset not found at %s. Please ensure dataset is downloaded.", csv_path)
        return 1
